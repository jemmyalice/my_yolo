...
class BaseTrainer:
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
        ...
        with torch_distributed_zero_first(LOCAL_RANK):  # 避免多次自动下载数据集
            ...
            # 新增部分
            if isinstance(self.trainset, list) and isinstance(self.testset, list):
                self.infusion = True
        self.ema = None
...
# 这里会根据最后生成的模型配置进行预测得到一系列图 画图
    ...
    def resume_training(self, ckpt):
        ...
        if self.ema and ckpt.get("ema"):
            # ckpt["ema"].float().state_dict() 获取了 ema 模型的所有参数，并将其格式化为适合加载到目标模型的形式。
