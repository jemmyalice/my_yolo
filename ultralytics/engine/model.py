...
class Model(nn.Module):
    ...
    def train(
        ...
        **kwargs,
        ...
        Args:
            trainer (BaseTrainer | None)：用于模型训练的自定义训练器实例。如果为 None，则使用默认值。
            **kwargs (Any)：用于训练配置的任意关键字参数。常用选项包括：
            data (str)：数据集配置文件的路径。
            epochs (int)：训练 epoch 的数量。
            batch_size (int)：用于训练的批次大小。
            imgsz (int)：输入图像大小。
            device (str)：运行训练的设备（例如“cuda”、“cpu”）。
            worker (int)：用于数据加载的工作线程数。
            optimizer (str)：用于训练的优化器。
            lr0 (float)：初始学习率。
            patient (int)：等待没有明显改进以提前停止训练的 epoch。
        ...
        # 如果没有指定 "cfg" 参数，使用默认配置 self.overrides
        overrides = yaml_load(checks.check_yaml(kwargs["cfg"])) if kwargs.get("cfg") else self.overrides
        custom = {
            # NOTE: handle the case when 'cfg' includes 'data'.
            #如果 overrides 中没有 data，则使用全局默认配置。
