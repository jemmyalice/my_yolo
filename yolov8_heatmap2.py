...
class yolov8_heatmap:
    ...
    def process(self, img_path, save_path):
        ...
        if self.renormalize:
            cam_image = self.renormalize_cam_in_bounding_boxes(pred[:,
                                                               :4].cpu().detach().numpy().astype(np.int32), img, grayscale_cam)
        if self.show_box:
            for data in pred:
...
def get_params():
    ...
    layers = [10, 12, 15, 18, 22]
    for grad_name in grad_list:
        params = {
            'weight': r'F:\downloads\chorme\best (2).pt',  # 训练好的权重路径
